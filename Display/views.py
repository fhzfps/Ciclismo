    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                treino.duracao_s=round(relatorio['duracao_s'],2)
                treino.NP=round(relatorio['NP'],2)
                treino.IF=round(relatorio['IF'],2)
                treino.PM=round(relatorio['PM'],2)
                treino.TTS=round(relatorio['TTS'],2)
                treino.CM=round(relatorio['CM'],2)
                treino.PMax=round(relatorio['PMax'],2)
                treino.PMin=round(relatorio['PMin'],2)
                treino.CMax=round(relatorio['CMax'],2)
                treino.CMin=round(relatorio['CMin'],2)
                treino.Calorias=round(relatorio['Calorias'],2)
                treino.Distancia=round(relatorio['Distancia'],2)
